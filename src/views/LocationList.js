import LocationActions from "components/LocationActions/LocationActions";
import React from "react";
import { useState, useEffect } from "react";
import { Card, Container, Row, Col, Form, FormControl } from "react-bootstrap";
import DataTable from "react-data-table-component";
import { Link } from "react-router-dom";
import { adminLocationsListService } from "services/admin.service";
...
function LocationList() {
  ...
  const getUserList = async () => {
    let response = await adminLocationsListService(page,searchText);
  ...
  const columns = [
    {
      ...
      {
        name: "Actions",
        cell: (row) => <LocationActions reload={reload} row={row} />,
      },
  ];
