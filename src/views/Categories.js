...
import { adminCategoriesListService } from "services/admin.service";
...
function Categories() {
  ...
  const getUserList = async () => {
    let response = await adminCategoriesListService(page,searchText);
  ...
  const columns = [
    ...
    },
    {
      ...
      {
        name: "Pice",
        selector: (row) => row.price || "Untitled",
        sortable: false,
      },
      {
        name: "Show in premium?",
        selector: (row) => row.isPremium === 1 ? 'Yes'  :  "No",
        sortable: false,
      },
    
    {
      name: "Actions",
      cell: (row) => <CategoriesActions reload={reload} row={row} />,
    },
  ];
